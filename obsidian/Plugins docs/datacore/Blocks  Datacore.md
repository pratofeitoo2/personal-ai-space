---
title: Blocks | Datacore
source: https://blacksmithgu.github.io/datacore/data/blocks
author:
published:
created: 2025-12-07
description: Datacore tracks every markdown block in markdown files and in canvas files. The metadata available depends on the type of block.
tags:
  - clippings
---
...
| --- | --- | --- |
| `$type` | The *type* of block - such as a list block or codeblock, etc. | `list` or `codeblock` or `datablock` or `paragraph` or `yaml` |
...
| **Field** | **Description** | **Example** |
| --- | --- | --- |
| `$type` | The *type* of list item - either `task` or `list`. | `task` or `list` |
...
| `$blockId` | If set, the block ID that can be used to link to this specific list item. | An Obsidian *block ID* like `ssa82hr` |
