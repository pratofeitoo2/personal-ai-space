---
title: Metadata | Datacore
source: https://blacksmithgu.github.io/datacore/data
author:
published:
created: 2025-12-07
description: Datacore is a metadata index - it stores information about every page, section, block, list item, canvas file, and other file in your vault
tags:
  - clippings
---
...
Most fields in the metadata start with a dollar sign (`$`), meaning they are *intrinsic* - automatically provided by Obsidian and Datacore. This includes things like a files path, tags and links in the file, titles, sections, and so on. Explicit properties you put in your `Properties` block or via inline fields are *user* metadata - `length`, `rating`, and `time-played` in the example above.
...
All objects are tagged with the file path (`$file`) that they came from. For objects that *are* files, like markdown pages, images, or canvas files, this data is also available as `$path`.